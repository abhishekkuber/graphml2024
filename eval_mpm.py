...
from eval_hybrid import ensure_dir
from utils import *
import numpy as np
...
def get_embedding(vectors: dict, x):
    matrix = np.zeros((x, len(list(vectors.values())[0])))
...
def eval(pos_train_path, neg_train_path, all_test_path, mirna_sim, disease_sim, mirna_emb, disease_emb):
    ...
    for pos_test_path in all_test_path:
        ...
        for testrate in neg_rates:
            ...
            for testset in range(10):
                ...
                # Calculate average accuracy and loss for plotting
                avg_acc += acc
                avg_loss += 1 - acc  # Assuming loss is the inverse of accuracy for simplicity
    ...
    return all_scores, test_losses, test_accuracies
...
def main():
    ...
    all_scores, test_losses, test_accuracies = eval(pos_train_path, neg_train_path, all_test_path, mirna_sim, disease_sim, mirna_emb, disease_emb)
    for line in all_scores:
        writer.write(','.join(line))
        writer.write('\n')
    writer.close()

    ensure_dir('plots/mpm')

    # Plotting the losses and test accuracy for MPM
    plt.figure()
    plt.plot(test_losses, label='Test Loss')
    plt.xlabel('Evaluation Step')
    plt.ylabel('Loss')
    plt.title('Test Loss for MPM')
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/mpm/mpm_test_loss.png')

    plt.figure()
    plt.plot(test_accuracies, label='Test Accuracy')
    plt.xlabel('Evaluation Step')
    plt.ylabel('Accuracy')
    plt.title('Test Accuracy for MPM')
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/mpm/mpm_test_accuracy.png')

    np.save('plots/mpm/mpm_test_losses.npy', test_losses)
    np.save('plots/mpm/mpm_test_accuracies.npy', test_accuracies)
