        ...
        for pos_test_path in all_test_path:
            ...
            for testrate in neg_rates:
                for testset in range(10):
                    ...
                    auc_score, ap_score, sn, sp, acc, prec, rec, f1, mcc = get_all_score(test_lbl, test_pred_lbl)  # TODO: added mcc (was missing)
                    tmp_score = [auc_score, ap_score, sn, sp, acc, prec, rec, f1, mcc]  # TODO: added mcc (was missing)
...
def main():
    ...
    for nfeat in all_nfeats:
        ...
        if not os.path.exists('./data/sota/'):
            os.mkdir('./data/sota/')

        writer = open('./data/sota/mpm_nem_' + str(nfeat) + '_' + args.pcg_setup + '_mar30_ind.txt', 'w')
        writer.write('data,run,auc_score, ap_score, sn, sp, acc, prec, rec, f1\n')

        dir = './data/new_disease/'
        pos_train_path = './data/hmdd2_pos.csv'
        neg_train_path_ori = './data/hmdd2_neg1.csv'
        all_test_path = ['./data/held_out1_pos.csv', './data/held_out2_pos.csv', './data/new_mirna_pos.csv']
